...
(function() {
  var markupify, markupify_single, timeify;

  timeify = function(tstr) {
    return moment.utc(tstr, "YY-MM-DD H:m:s").local().format("MMM D, hh:m A");
  };

  markupify_single = function(entry) {
    entry = $(entry).html().split(" - ");
    return "<p class='entry'>" + ("<span class='time'>" + (timeify(entry[0])) + "</span>") + ("<span class='author'>" + entry[1] + ":</span>") + ("<span class='msg'>" + entry.slice(2) + "</span>") + "</p>";
  };
